...
def search_breed(breed_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if "data" in parsed and parsed["data"] is not None:
                return (parsed, url)
        return None
    except Exception:
        return None
...
def get_longest_lifespan_breed(cache_file):
    ...
    for url, entry in cache.items():
        try:
            ...
            # Must be an integer (not a string like "long")
            if not isinstance(max_life, int):
                continue
            if best_lifespan is None or max_life > best_lifespan or (max_life == best_lifespan and name < best_name):
                best_name = name
